...
> | Simply Swords | 62 — 고유 무기 이름 + 무기 툴팁 문구 |
...
> 툴팁이 반쪽이 되기 때문이다.
>
> **아직 영어인 것:** Epic Knights 의 **방어구 245종**(이 모드는 아이템 463개 중 무기가 218개다),
> Chipped 7,265 · Rechiseled 3,656(블록 변형 스팸 — 손으로 쓸 물건이 아니라 규칙 생성이 맞다).
> 필요하면 같은 도구에 표만 더하면 된다.
>
> **도구가 스스로 신고한다.** 못 옮긴 항목이 있으면 종료 코드 2 로 목록을 찍고, 모드 업데이트로
> 키가 바뀌어 **안 쓰이게 된 번역**도 따로 보고한다. 조용히 빠지면 「다 됐다」로 읽히기 때문이다.
