...
//   수호 42칸 · 개척 38 · 창병 34 · 힐러 30 · 암살 30 · 헤카테 28 · 거너/사냥꾼/현자 26
...
const AS_HEALTH_BY_FATE = {
  ...
  hecate:   { 1: 0, 2: 8,  3: 18, 4: 26, 5: 36 }, // 10/14/19/23/28칸 — 중거리 지원
  // 하르모니아는 «원거리 지원 딜러»라 힐러(30칸)와 원거리 딜러(26칸) 사이에 둔다.
  // 원거리로 서지만 결속의 매듭을 깔고 그 안을 지켜야 해서 딜러 셋처럼 완전히 빠질 수가 없다
  // — 그 차이가 2칸이다. 헤카테(28칸)와 같은 자리인데 이유는 반대다(그쪽은 붙어야 해서).
  harmonia: { 1: 0, 2: 8,  3: 16, 4: 26, 5: 36 }, // 10/14/18/23/28칸 — 원거리 지원 딜러
  // 네메시스는 두 번째 탱커다. 아틀라스(42칸)와 크라토스(38칸) 사이에 둔다 — 앞에 서긴 서되
  ...
  // 자기 회복이 C「가르침」 8초뿐이라는 점도 여기 4칸에 얹혀 있다.
  chiron:   { 1: 0, 2: 10, 3: 22, 4: 34, 5: 48 }, // 10/15/21/27/34칸 — 근접 하이브리드 힐러
