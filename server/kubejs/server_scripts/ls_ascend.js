...
//
//   1성  유물 획득 시 기본        패시브 + 기본 스킬 (우클릭) · 체력 16~20칸(역할별)
...
// 체력이 그대로면 후반 보스에게 한 대 맞고 죽는 그림이 나온다.
// 앞라인일수록 두껍고 원거리/마법일수록 얇게 — 각성할수록 역할이 벌어진다.
// 값은 "추가 HP" = (목표 칸수 × 2) − 20. 주석의 숫자가 실제 도달 칸수.
//
// ── 1성 바닥을 올렸다 (2026-08-11, 유저 결정) ──
// 예전엔 **전원 10칸에서 출발**했다. 초반이 그만큼 물렀다 — 유물을 막 받은 시점이
// 이 팩에서 제일 얇은 구간인데, 몹 스케일링과 공성은 그때도 이미 돌고 있다.
// 이제 역할별로 바닥이 셋이다:
//     탱커 20칸 (이지스·아드라스테이아·타이탄)
//     근접 18칸 (게볼그·펠리온)
//     그 밖 16칸 (스틱스·파나케이아·헤스페로스·바르비톤·원거리 셋)
//
// ⚠️ **5성은 한 칸도 안 건드렸다.** 그쪽은 보스 공격력(`/bossdiff dmg`)에 맞춰 잰 값이라
//    올리면 그 균형이 같이 흔들린다. 바뀐 건 1~4성뿐이고, 5성 값은 이전과 완전히 같다.
//
// ⚠️ **대가: 각성의 체력 체감이 줄어든다.** 이지스는 32칸 성장 → **22칸**,
//    원거리 셋은 16칸 → **10칸** 이다. 「각성하면 두꺼워진다」가 그만큼 약해진 것을
//    알고 고른 값이다 — 초반의 물렁함을 그 값으로 샀다.
const AS_HEALTH_BY_FATE = {
  // ── 탱커 셋: 1성 20칸 ──
  guardian: { 1: 20, 2: 30, 3: 42, 4: 52, 5: 64 }, // 20/25/31/36/42칸 — 절대 탱커
  // 네메시스는 두 번째 탱커다. 아틀라스(42칸)와 크라토스(38칸) 사이 —
  // 「막아서 안 맞는」 이지스보다는 얇아야 패링을 맞추는 값어치가 생긴다.
  nemesis:  { 1: 20, 2: 30, 3: 40, 4: 50, 5: 60 }, // 20/25/30/35/40칸 — 패링 탱커
  pioneer:  { 1: 20, 2: 28, 3: 38, 4: 46, 5: 56 }, // 20/24/29/33/38칸 — 근접 탱커

  // ── 근접 둘: 1성 18칸 ──
  lancer:   { 1: 16, 2: 24, 3: 32, 4: 40, 5: 48 }, // 18/22/26/30/34칸 — 근접 브루저
  // 케이론은 «앞에 서는 힐러»다. 히기에이아(30칸)보다 두껍고 탱커(40·42칸)보다 얇다 —
  // 때려야 힐이 나오므로 뒤에 못 서고, 그렇다고 전선을 대신 맡을 수는 없어야 한다.
  // 게볼그와 시작·끝이 같고 중간만 한 칸 얇다(자기 회복이 「가르침」 8초뿐인 몫).
  chiron:   { 1: 16, 2: 22, 3: 30, 4: 38, 5: 48 }, // 18/21/25/29/34칸 — 근접 하이브리드 힐러

  // ── 그 밖 일곱: 1성 16칸 ──
  assassin: { 1: 12, 2: 18, 3: 26, 4: 32, 5: 40 }, // 16/19/23/26/30칸 — 근접 딜러
  // ※ 힐러는 가호 패시브로 최대 체력 +4(2칸)를 따로 받으므로 최종 32칸이 된다.
  healer:   { 1: 12, 2: 18, 3: 26, 4: 32, 5: 40 }, // 16/19/23/26/30칸 — 서포터(+가호 2칸)
  // 헤카테는 중거리라 암살(30칸)보다 얇고 원거리(26칸)보다는 두껍다. 저주를 쌓으려면
  // 평타 사거리 안까지 들어가야 하는데, 원거리 셋과 같은 체력이면 그 접근 자체가 불가능해진다.
  hecate:   { 1: 12, 2: 18, 3: 24, 4: 30, 5: 36 }, // 16/19/22/25/28칸 — 중거리 지원
  // 하르모니아는 «원거리 지원 딜러»라 힐러(30칸)와 원거리 딜러(26칸) 사이에 둔다.
  // 원거리로 서지만 결속의 매듭을 깔고 그 안을 지켜야 해서 딜러 셋처럼 완전히 빠질 수가 없다.
  // 헤카테(28칸)와 같은 자리인데 이유는 반대다(그쪽은 붙어야 해서).
  harmonia: { 1: 12, 2: 18, 3: 24, 4: 30, 5: 36 }, // 16/19/22/25/28칸 — 원거리 지원 딜러

  gunner:   { 1: 12, 2: 16, 3: 22, 4: 26, 5: 32 }, // 16/18/21/23/26칸 — 원거리 딜러
  hunter:   { 1: 12, 2: 16, 3: 22, 4: 26, 5: 32 }, // 16/18/21/23/26칸 — 원거리 딜러
  sage:     { 1: 12, 2: 16, 3: 22, 4: 26, 5: 32 }  // 16/18/21/23/26칸 — 마법 유리대포
}
// 가호 미선택/미확인 시 기준선 (16/19/22/26/30칸) — 「그 밖」 무리와 같은 바닥에서 출발한다
const AS_HEALTH_DEFAULT = { 1: 12, 2: 18, 3: 24, 4: 32, 5: 40 }
...
function asHealth(server, uname) {
  ...
  // 16칸이 된다.** 체력은 유물의 값이지 접속의 값이 아니다.
  if (!LS.hasRelic(server, uname)) {
    try { asCmd(server, `attribute ${uname} minecraft:generic.max_health modifier remove ${AS_HP_MOD}`) } catch (e) { lsWarn('ls_ascend:norelic', e) }
    return 0
  }
