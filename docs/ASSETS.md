...
11번째 가호([CLASSES.md](CLASSES.md) 「네메시스」)용 대검. **`Grand Claymore` — nongko's Fantasy Weapons.**
...
13번째 가호([CLASSES.md](CLASSES.md) 「케이론」)용 봉. **리소스팩을 안 뒤졌다.**
...
`item/handheld` → `item/generated` → **`builtin/generated`** 로 이어지는데,
그 로더는 **`elements` 를 통째로 무시하고** `layer0` 스프라이트에서 모델을 «생성»한다.
`layer0` 이 없으니 결과가 «빈 모델»이 된다. **오류도 경고도 안 난다** —
그래서 원인을 찾기가 특히 어렵다.

**나머지 유물 모델(nemesis · hecate · harmonia · sage)은 전부 `parent` 가 없다.**
