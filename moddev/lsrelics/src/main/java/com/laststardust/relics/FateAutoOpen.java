...
public final class FateAutoOpen {
    ...
    //
    // ⚠️ **한 줄에 한 문장.** 그리고 **한 줄은 25자 안쪽**이다. 이 둘은 같은 규칙이다 —
    //    줄이 길면 채팅이 알아서 접는데, 그 접힘은 문장을 아무 데서나 자른다
    //    (「…세계는 그 / 빛 아래에서」). 여기서 아무리 예쁘게 끊어도 소용이 없다.
    //
    //    접히는 폭은 **플레이어마다 다르다** — 채팅 폭 설정과 GUI 배율이 각자 다르기 때문이다.
    //    그래서 「내 화면에서 안 접히더라」는 근거가 못 된다. 짧게 쓰는 것만이 확실하다.
    //
    //    ⚠️ 그래서 **문장이 길면 문장을 쪼갠다** — 줄을 쪼개는 게 아니다.
    //       예전엔 긴 문장 하나를 두 줄로 접어 뒀는데, 그게 지금 고치고 있는 바로 그 증상이었다.
    //
    ...
    private static final String[][] PROLOGUE = {
        ...
        {"§7하늘에 하나만 남았을 때, 더는 아무도 말하지 않았다.",
         "§7밤이 낮을 밀어내고 있었다.",
         "§7땅이 갈라진 자리마다 빛이 닿지 않는 틈이 벌어졌다.",
         "§7그 안에서 무언가가 기어 나왔다.",
         "§7별을 삼킨 것이 이번에는 세계를 삼키러 온 것이다."},
        {"§7그리고 마지막 밤. 그 하나마저 떨어졌다.",
         "§7별은 하늘을 가로질러 이 땅 어딘가에 부딪혀 부서졌다.",
         "§7세계는 어둠에 잠겼다."},
    ...
    private static final int LINE_TICKS = 30;    // 1.5초 — 한 줄 읽는 시간 (줄이 짧아져 낮췄다)
     ...
     *
     * <p>⚠️ <b>대본은 첫 접속과 똑같은 것을 쓴다.</b> 예전엔 여기만 {@code WITH_PROLOGUE} 로
     * 박혀 있어서, 온보딩용으로 화면 열기를 뺀 뒤에도 <b>다시 보기에서는 선택창이 떴다</b> —
     * 확인하려고 만든 명령이 실제와 다른 것을 보여주면 계기가 아니라 거짓말이 된다.
     */
    public static void replay(ServerPlayer player) {
        ...
        tag.remove(K_SEEN);
        root.put(ServerPlayer.PERSISTED_NBT_TAG, tag);
        PENDING.add(new Pending(player, DEFER_TO_ONBOARDING ? PROLOGUE_ONLY : WITH_PROLOGUE));
    }
}
