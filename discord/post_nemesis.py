...
CLASS = {
    ...
    "rows": [
        ...
        ("우클릭", "흘리기", "방패처럼 쥐고 있는다 — 쥔 뒤 0.4초 안에 맞으면 완벽 패링(무효 + 경직 + 기세) · 그 뒤로는 앞에서 오는 피해 −15%", "—"),
        ("패시브", "강철의 각오", "방어력 +5 · 방어 강도 +3 · 평타 위협도 ×1.5 · 패링에 성공하면 기세가 쌓인다(최대 5중첩, 10초) — 쌓아두기만 해선 아무 값어치가 없다. 어디에 태우느냐가 전부다", ""),
        ("기본", "강철 발", "3초 뿌리내림 — 6칸 도발 2초 · 이동 불가 · 기세를 전부 소모해 받는 피해 −16%~−41% · 끝날 때 반경 5칸 밀어내며 피해", "12초"),
        ("이동", "참격 인계", "대검을 던지고 그 대검을 잡으며 날아간다 · 지나간 자리의 적이 베인다", "9초"),
        ("추가", "불굴", "8칸 도발 4초 · 기세를 전부 소모해 6초간 받는 피해 −10%+3%/중첩(최대 −25%) · 반경 4.5칸 충격 · 기세가 없어도 쓸 수 있다", "22초"),
        ("궁극", "일도양단", "대검을 치켜들었다 내려찍는다(0.8초) · 전방 12칸 직선 대형 일격 · 경직 + 방어력 절반 · 기세를 전부 태워 피해 +11%/중첩(5중첩 ×1.55) · 이후 5초간 주변 8칸 아군 받는 피해 −25%", "90초"),
    ],
}
