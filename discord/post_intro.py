...
#
# 디스코드에 처음 들어온 사람이 보는 «한 장짜리 목록». 가호의 이름·별칭·역할만 있고
# **스킬과 수치는 없다** — 그건 각 가호의 상세 채널에 있고, 그 채널은 가호를 고른 뒤에 열린다.
#
# ── 이름·역할을 여기에 다시 적지 않는 이유 ──
# post_atlas.py … post_chiron.py 의 CLASS 에 이미 icon/name/en/epithet/role 이 있다.
...
#    실제 선택은 인게임에서 하고, 거기엔 「한 서버에 같은 직업 둘 금지」 규칙이 있다
#    (HeroData.ownerOf). 디스코드는 그 규칙을 강제할 수 없으므로 문구로 분명히 해 둔다 —
#    안 그러면 둘이 같은 가호를 눌러놓고 인게임에서 한 명이 거절당한다.
#
#    ※ 2026-08-11: 「게임 안에서 `/fate` 로 정하며」였는데 이제 **명령어를 칠 일이 없다.**
#      처음 접속하면 프롤로그가 흐른 뒤 선택 화면이 자동으로 뜬다(FateAutoOpen).
#      명령어를 안내하면 새 사람이 「그걸 쳐야 하나」 하고 헤맨다.
GUIDE = [
    "-# 아래에서 가호를 고르면 그 가호의 **상세 채널**이 열립니다. 유물·스킬·성장은 그곳에 있습니다.",
    "-# 여기서 고르는 것은 «읽을 채널을 여는 것»입니다. 실제 직업은 **서버에 처음 들어가면 뜨는 "
    "선택 화면**에서 정하며, 같은 직업은 한 사람만 가질 수 있습니다.",
]
