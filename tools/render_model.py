...
#
# ── 2026-08-09: north↔west · south↔east 가 뒤바뀌어 있었다 ──
# `corners()` 의 비트는 (1=x, 2=z, 4=y) 다. 그래서 예전 'north' 인덱스 (0,2,6,4) 는
# **x 가 고정된 면**이었다 — 마크에서 north 는 −Z 면인데 여기선 −X 면을 그리고 있었다.
# 여섯 면이 다 있는 보통 상자에서는 «다른 면의 UV 가 서로 바뀌어» 붙을 뿐이라 형태가 맞아
# 눈치채기 어려웠는데, **두께 0인 평면**에서 정체가 드러났다:
# 헤스페로스 el14~18(`from.x == to.x`)은 인게임에서 east/west 두 면만 보이고 나머지 넷은
# 넓이가 0이라 안 보인다. 그런데 이 표가 뒤집혀 있어서 렌더러는 그 «안 보여야 할» north/south
# 를 큼직하게 그렸고, 그 면들의 UV 는 블록벤치가 남긴 쓰레기 값(`[0,0,0,x]` = 텍스처 0번 열)
# 이었다. 결과로 **인게임에 없는 얼룩덜룩한 띠**가 날 뒤쪽에 나타났다.
FACES = {
