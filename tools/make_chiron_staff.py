...
def orb(cy):
    ...
    """
    return (pillar(0.50, cy - 0.92, cy - 0.52, U_GOLD, U_GOLD_D)
            + pillar(0.74, cy - 0.52, cy + 0.52, U_GOLD, U_GOLD_D)
            + pillar(0.50, cy + 0.52, cy + 0.92, U_GOLD, U_GOLD_D))


# ── 치수 ──
# ⚠️ 마인크래프트 아이템 모델은 좌표를 **−16 ~ 32** 까지 허용한다. 0~16 에 가두면
#    「길다」가 안 나온다 — 두께가 1 인데 길이가 15 면 비율이 1:15 뿐이다.
#    여기서는 −6 ~ 22 (28칸) 를 써서 뽑는다.
#    ⚠️ 2차 조정(유저): 자루를 0.55 → 0.42 로 얇게. 구도 1.50 → 1.12 로 줄였다 —
#       구가 크면 «지팡이 머리»로 읽혀서 우라니아 쪽으로 실루엣이 끌려간다.
#    ⚠️ 3차 조정(유저): 자루 0.42 → **0.34** (비율 1:41) · 남색 구간 9.2 → 7.0.
#    ⚠️ 4차 조정(유저): 구 1.12 → 0.90 · 남색 7.0 → 6.0.
#    ⚠️ 5차 조정(유저): 구 0.90 → **0.74** · 남색 6.0 → **5.0**.
#       구가 금색 마디(0.52)의 1.4배까지 내려왔다 — **여기가 «구»로 읽히는 한계**다.
#       더 줄이면 마디 세 개가 나란히 있는 꼴이 되고 끝이 «끝»으로 안 보인다.
#       자루가 얇아진 만큼 구가 «상대적으로» 커 보이던 것을 여기서 맞췄다.
#       ⚠️ 이 근처가 하한이다 — 자루 0.68 은 텍스처 픽셀 한 칸 수준이라 더 얇으면
#          3인칭에서 각도에 따라 «사라진다».
Y_BOT, Y_TOP = -6.0, 22.0
ORB_B, ORB_T = -4.4, 20.4          # 구의 중심 (양 끝에서 1.6 안쪽)
BAND_B0, BAND_B1 = 4.3, 5.5        # 아래 금색 마디
BAND_T0, BAND_T1 = 10.5, 11.7      # 위 금색 마디
SHAFT = 0.34                       # 자루 반두께
SLEEVE = 0.42                      # 남색 구간 (자루보다 살짝 굵게 = 감싼 느낌)
BAND = 0.52                        # 금색 마디
